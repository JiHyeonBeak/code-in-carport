...
public class shiftCipher {
    ...
    // 복호화 함수
    public void decodingFunction(String decodingWord) {
        // 복호화 편리를 위해, 입력받은 문자열을 대문자로 변경.
        String word = decodingWord.toUpperCase();
        // 아스키코드의 "A"값 설정.
        int aNum = 65;
        ...
        for (int i=0; i <= bytes.length-1 ; i++) {
            ...
            // 만약 A 이전의 문자라면, 다시 기준점을 Z로 초기화...
            if(getNum < 65) {
                int calNum = (65 - getNum)+aNum;
                bytes[i] = (byte)calNum;
            } else {
                bytes[i] = (byte)getNum;
            }
